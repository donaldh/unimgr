...
public class LegatoUtils {
	...
	public static final Map<String, Object> parseNodes(Evc evc) {
		...
		for (EndPoint endPoint : evc.getEndPoints().getEndPoint()) {
			...
			uniList.add(endPoint.getUniId().getValue().toString() + "#"
					+ vlanId);

		}

		evcMap.put(LegatoConstants.EVC_ID, evc.getEvcId().getValue());
		evcMap.put(LegatoConstants.EVC_MAX_FRAME, (evc.getMaxFrameSize()
				.getValue() != null) ? evc.getMaxFrameSize().getValue() : "");
		evcMap.put(LegatoConstants.EVC_STATUS,
				(evc.getStatus() != null) ? evc.getStatus() : "");
		evcMap.put(LegatoConstants.EVC_CON_TYPE, (evc.getConnectionType()
				.getName() != null) ? evc.getConnectionType().getName() : "");
		evcMap.put(LegatoConstants.EVC_UNI_LIST, uniList);

		return evcMap;
	}

	public static final EndPoint2 buildCreateEthConnectivityEndPointAugmentation(
			String vlanId) {
		return new EndPoint2Builder()
				.setNrpCarrierEthConnectivityEndPointResource(
						buildNrpCarrierEthConnectivityEndPointResource(vlanId))
				.build();
	}

	public static final EndPoint7 buildUpdateEthConnectivityEndPointAugmentation(
			String vlanId) {
	...
	public static final CreateConnectivityServiceInput1 buildCreateConServiceAugmentation(
			String maxFrameSize) {
		CreateConnectivityServiceInput1 createConServiceAugmentation = new CreateConnectivityServiceInput1Builder()
				.setNrpCarrierEthConnectivityResource(
						buildNrpCarrierEthConnectivityResource(maxFrameSize))
				.build();
		return createConServiceAugmentation;
	}

	public static final UpdateConnectivityServiceInput1 buildUpdateConServiceAugmentation(
			String maxFrameSize) {
	...
	public static final NrpCarrierEthConnectivityEndPointResource buildNrpCarrierEthConnectivityEndPointResource(
		...
		VlanIdBuilder vlanIdBuilder = new VlanIdBuilder()
				.setVlanId(new PositiveInteger(Long.parseLong(vlanId)));
		...
		nrpCarrierEthConnectivityEndPointResourceBuilder
				.setIngressBwpFlow(buildIngressBwFlow());
	...
	public static final IngressBwpFlow buildIngressBwFlow() {
		IngressBwpFlowBuilder ingressBwpFlowBuilder = new IngressBwpFlowBuilder()
				...
				.setCbs(new NaturalNumber(LegatoConstants.LONG_VAL))
				.setEir(new NaturalNumber(LegatoConstants.LONG_VAL))
				.setEirMax(new NaturalNumber(LegatoConstants.LONG_VAL))
				.setEbs(new NaturalNumber(LegatoConstants.LONG_VAL))
				.setCouplingFlag(true)
				.setColorMode(ColorMode.COLORAWARE)
				.setTokenRequestOffset(
						new NaturalNumber(LegatoConstants.LONG_VAL));

		return ingressBwpFlowBuilder.build();
	}

	public static final NrpCarrierEthConnectivityResource buildNrpCarrierEthConnectivityResource(
			String maxFrameSize) {
		...
		return nrpCarrierEthConnectivityResourceBuilder.setMaxFrameSize(
				new PositiveInteger(Long.parseLong(maxFrameSize))).build();
	}

	public static final CreateConnectivityServiceInput buildCreateConnectivityServiceInput(
			Map<String, Object> evcMap, String connType) {
		...
		switch(connType.replace("-", "").toUpperCase()){
			...
			case LegatoConstants.MULTIPOINTTOMULTIPOINT :
				...
				.setConnConstraint(new ConnConstraintBuilder()
						.setServiceLevel(LegatoConstants.BEST_EFFORT)
						.setServiceType(ServiceType.MULTIPOINTCONNECTIVITY)
						.build());
				break;
			default:
				break;
		...
		layerList.add(new LayerProtocolBuilder().setLocalId("eth")
		...
		createConnServiceInputBuilder.addAugmentation(
				CreateConnectivityServiceInput1.class,
				LegatoUtils.buildCreateConServiceAugmentation(evcMap.get(
						LegatoConstants.EVC_MAX_FRAME).toString()));

		return createConnServiceInputBuilder.build();
	}

	public static final UpdateConnectivityServiceInput buildUpdateConnectivityServiceInput(
			Map<String, Object> evcMap, String uniStr, String uuid, String connType) {
		...
		switch(connType.replace("-", "").toUpperCase()){
			case LegatoConstants.POINTTOPOINT :
				...
				.setConnConstraint(new org.opendaylight.yang.gen.v1.urn.onf.params.xml.ns.yang.tapi.connectivity.rev171113.update.connectivity.service.input.ConnConstraintBuilder()
						.setServiceLevel(LegatoConstants.BEST_EFFORT)
						.setServiceType(ServiceType.POINTTOPOINTCONNECTIVITY)
						.build());
				break;
		...
		updateConnServiceInputBuilder.setEndPoint(buildUpdateEndpoints(uniStr,
				layerList));
		updateConnServiceInputBuilder.addAugmentation(
				UpdateConnectivityServiceInput1.class,
				LegatoUtils.buildUpdateConServiceAugmentation(evcMap.get(
						LegatoConstants.EVC_MAX_FRAME).toString()));
	...
	private static List<org.opendaylight.yang.gen.v1.urn.onf.params.xml.ns.yang.tapi.connectivity.rev171113.create.connectivity.service.input.EndPoint> buildCreateEndpoints(
		...
		for (String uniStr : uniList) {
			...
			endPointBuilder = new EndPointBuilder()
					...
					.setLocalId("e:" + uniArr[0])
					.setServiceInterfacePoint(new Uuid(uniArr[0]))
					.setDirection(PortDirection.BIDIRECTIONAL)
					.setLayerProtocol(layerList)
					.addAugmentation(
							EndPoint2.class,
							LegatoUtils
									.buildCreateEthConnectivityEndPointAugmentation(uniArr[1]));
		...
		if (StringUtils.isNotBlank(uniStr)) {
			...
			endPointBuilder
					...
					.addAugmentation(
							EndPoint7.class,
							LegatoUtils
									.buildUpdateEthConnectivityEndPointAugmentation(uniArr[1]));
		}
	...
	public static final Optional<Evc> readEvc(DataBroker dataBroker,
			LogicalDatastoreType store, InstanceIdentifier<?> evcNode) {
		final ReadTransaction read = dataBroker.newReadOnlyTransaction();
		final InstanceIdentifier<Evc> evcId = evcNode
				.firstIdentifierOf(Evc.class);
		final CheckedFuture<Optional<Evc>, ReadFailedException> linkFuture = read
				.read(store, evcId);
		try {
		...
		try {
			...
			case LegatoConstants.SLS_PROFILES:
				final InstanceIdentifier<org.opendaylight.yang.gen.v1.urn.mef.yang.mef.global.rev171215.mef.global.sls.profiles.Profile> profileId = child
						.firstIdentifierOf(org.opendaylight.yang.gen.v1.urn.mef.yang.mef.global.rev171215.mef.global.sls.profiles.Profile.class);
				final CheckedFuture<Optional<org.opendaylight.yang.gen.v1.urn.mef.yang.mef.global.rev171215.mef.global.sls.profiles.Profile>, ReadFailedException> profileFuture = read.read(store, profileId);
			...
			case LegatoConstants.COS_PROFILES:
				final InstanceIdentifier<org.opendaylight.yang.gen.v1.urn.mef.yang.mef.global.rev171215.mef.global.cos.profiles.Profile> cosProfileId = child
						.firstIdentifierOf(org.opendaylight.yang.gen.v1.urn.mef.yang.mef.global.rev171215.mef.global.cos.profiles.Profile.class);
			...
			case LegatoConstants.BWP_PROFILES:
				...
				.firstIdentifierOf(org.opendaylight.yang.gen.v1.urn.mef.yang.mef.global.rev171215.mef.global.bwp.flow.parameter.profiles.Profile.class);
